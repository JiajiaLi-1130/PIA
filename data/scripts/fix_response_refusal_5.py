...
target_file = os.path.join("data", "train", "train1", "sft_persona_fixed.jsonl")
output_file = os.path.join("data", "train", "train1", "sft_persona_fixed1.jsonl")
...
with open(target_file, 'r', encoding='utf-8') as infile, open(output_file, 'w', encoding='utf-8') as outfile:
    for line in infile:
        ...
        try:
            ...
            is_safe = safety_assessment.get("is_safe", False)

            if not is_safe and prompt in prompt_to_response:
