...
matplotlib.rcParams['font.sans-serif'] = ['Calibri', 'DejaVu Sans', 'Arial']
...
def load_stats(file_path: Path) -> List[dict]:
    """Load list of dicts from a jsonl file. Ignore empty lines and parse errors."""
    stats = []
...
def plot_comparison(datasets: List[Tuple[str, str]], output: Path, show: bool = True, dpi: int = 300) -> None:
    ...
    # Use a clean white background style (avoid gray seaborn background)
    ...
    def extract_model_and_method(display_name: str):
        """Extract model and method from display name.

        Heuristic: treat tokens containing 'B' (e.g., '8B') as model size markers
        and include a following 'Instruct' token in the model name when present.
        Remaining tokens are treated as the method name.
        """
    ...
    # method -> color mapping (consistent color per method)
    method_color_map = {
        'Ours': '#1f77b4',
        'Persona-GA': '#d62728',
    }
    default_method_colors = ['#2ca02c', '#9467bd', '#8c564b', '#e377c2']
    ...
    # Assign colors to methods (fallback to default sequence)
    assigned_method_colors = {}
    for _name, _stats, _model, method in parsed:
        if method in method_color_map:
            assigned_method_colors[method] = method_color_map[method]
    ...
    # Assign markers to models: prefer consistent shapes for known models
    for _name, _stats, model, _method in parsed:
        if model not in model_marker_map:
            # Explicit mapping: use circle for Qwen, triangle for Llama.
            # Fallback: keep previous historic ordering for other models.
            m_lower = model.lower()
            if 'qwen' in m_lower:
                model_marker_map[model] = 'o'
            elif 'llama' in m_lower:
                model_marker_map[model] = '^'
            else:
    ...
    for idx, (field, title_en, _title_cn) in enumerate(fields):
        ax = axes[idx]
        ...
        # Explicit font sizes: keep axis labels, remove subplot title
    ...
    # Compose legend: models first, then methods
    u_models = sorted(list(model_marker_map.keys()))
    u_methods = sorted(list(assigned_method_colors.keys()))

    # Model markers
    row1_h = [
        Line2D([0], [0], color='black', marker=model_marker_map[m], linestyle='', markersize=6, markeredgewidth=0.8)
        for m in u_models
    ]
    row1_l = u_models

    # Method lines
    row2_h = [Line2D([0], [0], color=assigned_method_colors[met], lw=2.5) for met in u_methods]
    row2_l = u_methods

    final_handles = row1_h + row2_h
    final_labels = row1_l + row2_l
...
def default_datasets() -> List[Tuple[str, str]]:
    # Use environment variable to avoid embedding internal paths
    base = os.environ.get('ATTACK_BASE', '  ')
    # Anonymize model display names for open-source release
    return [
        ...
        (f'{base}/base1/generation_stats.jsonl', 'ModelA-Instruct-Persona-GA'),
        (f'{base}/base2/generation_stats.jsonl', 'ModelB-Instruct-Persona-GA'),
    ]

def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Plot comparison for four dataset groups')
    p.add_argument('--output', '-o', default='result/attack/comparison_four_datasets.png', help='output image path')
    p.add_argument('--no-show', action='store_true', help='do not display the figure')
    p.add_argument('--dpi', type=int, default=300, help='output DPI for raster images')
    return p.parse_args()
